...
In order to do anything with the EIDITH R package, you need to first make sure to load the `eidith` library. For straightforward data manipulation we will also load the `tidyverse` package:
...
#### Automatically Grouped FASTA files by Viral Family or Method:  

The function `eidith_fasta_group()` will automatically create FASTA files that have been grouped by either Viral Family or Method. The name of these files will correspond to the grouping variable. If we want to output grouped FASTA files based on Viral family, we can run:
