...
If we run `names(mock_ea)` we can see that we now have a dataframe with all the columns from both the `events` and `animals` tables. Our next step is to join this table to the specimens table:
```{r 5}
