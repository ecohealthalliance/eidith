...
you have question, bug reports, or feature requests, please post them on our
...
require the [**devtools**](https://CRAN.R-project.org/package=devtools) package Use the following command to install the package.
...
`download_db()` or the functions `get_*` from the R console, your EIDITH username
and password will be requested.  To use these functions in scripts, you 
must provide these as *environent variables*.  

You can cache your credentials to logon automatically by putting them in a hidden
`.Renviron` file, which defines environment variables for your R session. For
more information see the `?eidith_auth` help file.
...
the `download_db()` command, like so:
...
`download_db()` may be run again to update your local database with the latest
...
`download_db()` creates), you can use the `get_*` functions, such as `get_ed_events()`.
