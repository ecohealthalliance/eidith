...
pp_Specimen <- function(dat) {
  dat <- dat %>%
    mutate(CorrectedSpecimenIDUnique = str_replace(dat$SpecimenIDUnique, "Lung, Liver", "Lung-Liver")) %>%
    mutate(CorrectedSpecimenIDUnique = str_replace(CorrectedSpecimenIDUnique, "kushtia", "Kushtia")) %>%
    mutate(CorrectedSpecimenIDUnique = despace(CorrectedSpecimenIDUnique)) %>%
    mutate(CorrectedSpecimenIDUnique = destrange(CorrectedSpecimenIDUnique)) %>%
    mutate(CorrectedSpecimenIDUnique = reduce_dashes(CorrectedSpecimenIDUnique))
  return(dat)
}
...
pp_Test <- function(dat){
  dat <- dat %>%
    mutate(CorrectedSpecimenNames = str_replace_all(dat$SpecimenName, "Lung, Liver", "Lung-Liver")) %>%
    mutate(CorrectedSpecimenNames = str_replace_all(CorrectedSpecimenNames, "110818EKSGbg001", "110818EKSGBG001")) %>%
    mutate(CorrectedSpecimenNames = str_replace_all(CorrectedSpecimenNames, "120716EKSGbb001", "120716EKSGBB001")) %>%
    mutate(CorrectedSpecimenNames = str_replace_all(CorrectedSpecimenNames, "mx", "MX")) %>%
    mutate(CorrectedSpecimenNames = str_replace_all(CorrectedSpecimenNames,"^78$", "078a")) %>%
    mutate(CorrectedSpecimenNames = decommaspace(CorrectedSpecimenNames)) %>%
    mutate(CorrectedSpecimenNames = despace(CorrectedSpecimenNames)) %>%
    mutate(CorrectedSpecimenNames = destrange(CorrectedSpecimenNames)) %>%
    mutate(CorrectedSpecimenNames = reduce_dashes(CorrectedSpecimenNames))
  return(dat)
}

destrange <- function(c){
  str_replace_all(c, "1,2$","1-2")
}
